...
- **Untyped `MorphTo` in dot notation keeps its WHERE but skips relevance ranking.** A column like `'commentable.title'` where `commentable` is a `MorphTo` still searches via `whereHas` (Eloquent resolves the morph's present target types from the morph map, and it's a harmless `0 >= 1` no-op where it can't) - so its matching behaviour is unchanged from before relevance ordering existed. Only the relevance `ORDER BY` skips it: `relevanceScoreFor` returns `null` for a `MorphTo` because an unconstrained `getRelated()` returns the parent, so grading it would build a subquery against the wrong table (the SQL error 1.1.0 introduced). `isMorphToColumn()` makes that one decision; `isExternalRelation` also excludes `MorphTo` (it extends `BelongsTo`) so the WHERE routes through `whereHas`, not the external `pluck`. Use the typed morph syntax `'commentable:post.title'` to *also* get relevance ranking on a morph target.
